...
Expanded JuniorPython automation:
- More output formats (blender_python, gltf_export)
- Smarter batching with format-specific optimization and error resilience.
"""
...
class CADScriptGenerator:
    def __init__(self, hybrid_quantizer: Optional[Any] = None):
        ...
        self.supported_formats = [
            "openscad", "python_cadquery", "step", "freecad",
            "optimization_script", "simulation_script", "blender_python", "gltf_export"
        ]
    ...
    def batch_generate_artifacts(self, design_states: List[Any], output_dir: str = ".", formats: Optional[List[str]] = None) -> Dict[str, int]:
        """
        Smarter batching: Generate multiple formats for multiple designs with error resilience.
        Returns count per format.
        """
        if formats is None:
            formats = ["python_cadquery", "step"]
        ...
        for i, ds in enumerate(design_states):
            for fmt in formats:
                try:
                    if fmt == "step":
                        filename = f"{output_dir}/design_{i}.step"
                        if self.generate_step(ds, filename):
                            counts[fmt] += 1
                    else:
                        filename = f"{output_dir}/design_{i}.{ 'py' if fmt == 'python_cadquery' else 'txt' }"
                        if self.export_to_file(ds, filename, format=fmt):
                            counts[fmt] += 1
                except Exception as e:
                    print(f"[CADScriptGenerator] Batch error on {fmt} for design {i}: {e}")
        return counts
    ...
    def subscribe_to_design_deliverables(self, graph_memory: Any, deliverable_type: str = "design"):
        ...
        graph_memory.subscribe_to_deliverable_type(deliverable_type, on_new_design)
        print("[CADScriptGenerator] Reactively subscribed to design deliverables.")
    ...
    def _auto_generate_from_deliverable(self, deliverable: Dict[str, Any]):
        ...
        if params:
            ...
            self.export_to_file(design_state, f"auto_{int(time.time())}.py", format="python_cadquery")
            self.export_to_file(design_state, f"auto_{int(time.time())}.step", format="step")
    ...
    def generate_blender_python(self, design_state: Any) -> str:
        """New format: Blender Python script for visualization/animation."""
        ...
        return f"""# Blender Python Script - Generated by JuniorCloud CADScriptGenerator
...
# Fuselage
bpy.ops.mesh.primitive_cylinder_add(radius=1.5, depth={q_params.get('length', 30)}, location=(0, 0, 0))

# Wing (simplified)
bpy.ops.mesh.primitive_cube_add(size=8, location=(0, 0, {q_params.get('length', 30)/2}))
wing = bpy.context.active_object
wing.rotation_euler[2] = {q_params.get('wing_sweep', 35)} * 3.14159 / 180

print("Blender model generated for supersonic design.")
"""

    def generate_gltf_export(self, design_state: Any, filename: str = "design.gltf") -> bool:
        """New format: Attempt GLTF export (requires Blender or pygltflib in real env)."""
        try:
            # Placeholder - in real deployment this would use pygltflib or Blender
            params = self._parse_design_params(design_state)
            with open(filename, "w") as f:
                f.write(f"# GLTF placeholder for design with sweep {params.get('wing_sweep')}\n")
            self._generation_count += 1
            return True
        except Exception as e:
            print(f"[CADScriptGenerator] GLTF export error: {e}")
            return False
...
"""
    ...
    def generate_script(self, design_state: Any, format: str = "openscad") -> str:
        ...
        elif format == "blender_python":
            return self.generate_blender_python(design_state)
        elif format == "gltf_export":
            # For gltf we prefer the dedicated method
            return "# Use generate_gltf_export() for actual file output"
        else:
    ...
    def export_to_file(self, design_state: Any, filename: str, format: str = "openscad") -> bool:
        try:
            ...
            if format == "gltf_export":
                return self.generate_gltf_export(design_state, filename)
...
if __name__ == "__main__":
    ...
    print(gen.generate_script(test_design, "blender_python"))
